...
<form style="margin-top:90px;" method="post" enctype="multipart/form-data" action="{{route('admin.product.editpost')}}">
...
@endif --}}
    ...
    <div class="col">
      <div class="form-outline">
        ...
        @endphp
        <input type="hidden" name="ma" value="{{$data->SP_MA}}">
        <input type="text" name='ten' id="form6Example1" value="{{$data->SP_TEN}}" placeholder="Tên" class="form-control" />
    ...
    <div class="col">
      <div class="form-outline">
        <input type="text" id="form6Example2" name="gia" placeholder="Giá" value="{{$data->SP_GIA}}" class="form-control" />
        @if($errors->has('gia')) 
  ...
  <img style="height:50px;"  src="{{ asset('storage/img/'.$data->SP_ANH) }}" alt="">
  ...
  <!-- Text input -->
  <div class="form-outline mb-2">
    <textarea class="form-control" id="exampleFormControlTextarea1" name="mota" rows="3" placeholder="Mô tả">{{$data->SP_MOTA}}</textarea>
  ...
  <select class="form-select" name="danhmuc" aria-label="Default select example">
    ...
    <option  value="{{$danhmuc->DM_MA}}" {{ $danhmuc->DM_MA == $data->DM_MA ? 'selected' : '' }}  >{{$danhmuc->DM_TEN}}</option>
